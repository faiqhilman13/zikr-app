...
/**
 * How many browsers visit Zikr, come back, and open it from a home screen.
 *
 * Nothing about what anyone counts reaches this endpoint. A participating browser sends a
 * random identifier it generated for itself and one of three facts about today: it opened
 * the site, it used the app after onboarding, it did so from a home-screen install. No
 * phrase, count, timer, target, language, page or referrer is accepted, and the payload
 * shape is checked exactly rather than merely parsed, so nothing else can be smuggled in.
 *
...
const KINDS = ['visit', 'active', 'standalone'];
...
/** Exactly two keys, an identifier the browser made up, and known kinds. Nothing else. */
export function validPayload(body) {
  return !!body && typeof body === 'object' && !Array.isArray(body)
    ...
    && body.kinds.every((kind) => KINDS.includes(kind));
}
...
export function summarize(keys, now = new Date()) {
  ...
  const records = keys
    .map((key) => key.split('/'))
    .filter(([date, kind, id]) => DATE.test(date) && date >= day(WINDOW_DAYS - 1) && date <= today && KINDS.includes(kind) && UUID.test(id));

  const ids = (kind, from, to = today) =>
    new Set(records.filter(([date, k]) => k === kind && date >= from && date <= to).map(([, , id]) => id));
  ...
  for (const [date, kind, id] of records) {
    if (kind !== 'visit') continue;
    if (!visitDays.has(id)) visitDays.set(id, new Set());
    visitDays.get(id).add(date);
  }
  ...
  return {
    ...
    timezone: 'UTC',
    visitors: ids('visit', day(WINDOW_DAYS - 1)).size,
    activeToday: ids('active', today, today).size,
    active7d: thisWeek.size,
    active30d: ids('active', day(WINDOW_DAYS - 1)).size,
    returning: [...visitDays.values()].filter((days) => days.size > 1).length,
    standalone: ids('standalone', day(WINDOW_DAYS - 1)).size,
    priorWeekActive: priorWeek.size,
    retainedFromPriorWeek: [...priorWeek].filter((id) => thisWeek.has(id)).length,
    daily: Array.from({ length: WINDOW_DAYS }, (_, index) => {
      const date = day(WINDOW_DAYS - 1 - index);
      return { date, visitors: ids('visit', date, date).size, active: ids('active', date, date).size, standalone: ids('standalone', date, date).size };
    })
  };
}
...
async function collect(request) {
  ...
  const usage = store();
  await Promise.all([...new Set(body.kinds)].map((kind) => usage.setJSON(`${date}/${kind}/${body.id}`, { v: 1 }, { onlyIfNew: true })));
...
// A participating browser sends at most three requests a day, so a per-IP ceiling this
