...
interface HeatCell {
  ...
  level: number;
  completed: boolean;
  /** Missed, but a streak freeze covered it. */
  frozen: boolean;
  active: boolean;
}

export function PracticeHeatmap({ state, frozenDays, onSelectDay }: { state: ZikrState; frozenDays: string[]; onSelectDay: (date: string) => void }) {
  const { t, i18n } = useTranslation();
  const target = Math.max(1, totalTarget(state));

  const { weeks, months, weekdays, anyFrozen } = useMemo(() => {
    ...
    const weeks = Array.from({ length: WEEKS }, (_, week) => Array.from({ length: 7 }, (_, row): HeatCell => {
      ...
      return { date: key, label: dayFormat.format(date), total, level, completed: log?.completed === true, frozen: frozen.has(key), active: total > 0 || timed > 0 };
    }));
    ...
    return { weeks, months, weekdays, anyFrozen: weeks.some((week) => week.some((cell) => cell.frozen)) };
  }, [state.logs, frozenDays, target, i18n.language]);
  ...
  return <section className="chart-card heatmap-card" aria-labelledby="heatmap-title">
      ...
      <div className="heatmap-grid">
        {weeks.map((week, index) => <div className="heatmap-week" key={index}>
          {week.map((cell) => cell.active
            ? <button key={cell.date} type="button" className={`heat-cell l${cell.level}${cell.completed ? ' done' : ''}${cell.frozen ? ' frozen' : ''}`} title={`${cell.label} · ${cell.total}`} aria-label={`${cell.label} · ${cell.total} ${t('repetitions')}${cell.frozen ? ` · ${t('frozenLegend')}` : ''}`} onClick={() => onSelectDay(cell.date)} />
            : <span key={cell.date} className={`heat-cell l${cell.level}${cell.frozen ? ' frozen' : ''}`} aria-hidden="true" />)}
        </div>)}
      </div>
    </div>
    <div className="heatmap-legend">
      ...
      {anyFrozen && <span className="legend-done"><i className="heat-cell l0 frozen" aria-hidden="true" />{t('frozenLegend')}</span>}
    </div>
  </section>;
}
