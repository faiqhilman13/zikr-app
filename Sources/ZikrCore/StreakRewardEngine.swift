...
public enum RewardEngine {
    public static func recalculate(
        ...
        let totalCount = history.reduce(0) { $0 + $1.totalCount }
        let peakCount = history.map(\.totalCount).max() ?? 0
        let xp = totalCount + (completedDays * 100) + (currentStreak.current * 25)
        ...
        if peakCount >= goal.targetCount * 2 {
        ...
        if totalCount >= 1000 {
            badges.append(.init(id: "thousand-club", title: "Thousand Club", detail: "Accumulated 1,000 dhikr counts.", iconName: "star.circle.fill"))
        }
